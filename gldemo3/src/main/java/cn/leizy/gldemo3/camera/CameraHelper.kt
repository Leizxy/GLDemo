package cn.leizy.gldemo3.camera
...
import cn.leizy.gldemo2.camera.getPreviewOutputSize
import cn.leizy.gldemo3.gl.CameraView
...
class CameraHelper(private val view: CameraView) {
...
//                    cameraList.add(CameraInfo(it, size, isFront))
    ...
    private fun initializeCamera(lifecycleOwner: LifecycleOwner) =
        lifecycleOwner.lifecycleScope.launch(Dispatchers.Main) {
            camera = openCamera(cameraManager, cameraId, cameraHandler).also { device ->
                ...
                if (targetSurfaces.isNotEmpty()) {
                    Log.i("CameraHelper", "initializeCamera:targetSurfaces ")
                    captureRequest = previewSurface?.let { surface ->
                        val build = device.createCaptureRequest(CameraDevice.TEMPLATE_PREVIEW)
                        build.addTarget(surface)
                        build.build()
                    }
                    session = createCaptureSession(device, cameraHandler)
                    startPreview()
                }
            }
        }
