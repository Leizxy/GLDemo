...
class YUVRender(private val view: CameraFilterView) : GLSurfaceView.Renderer,
    ...
    override fun onSurfaceCreated(gl: GL10?, config: EGLConfig?) {
        ...
        CameraHelper(view, analyzer = this)
...
/*
    ...
    private fun textureLuminanceAlpha(
...
/*        val y = ByteArray(planes[0].buffer.limit() - planes[0].buffer.position())
        val u = ByteArray(planes[1].buffer.limit() - planes[1].buffer.position())
        val v = ByteArray(planes[2].buffer.limit() - planes[2].buffer.position())
        ...
        yBuffer = bArray2bBuffer(y)
        uBuffer = bArray2bBuffer(u)
        vBuffer = bArray2bBuffer(v)*/
        if (!this::yBuffer.isInitialized) {
//            val ySize = imageWidth * imageHeight
            yBuffer = ByteBuffer.allocate(planes[0].buffer.limit() - planes[0].buffer.position())
            uBuffer = ByteBuffer.allocate(planes[1].buffer.limit() - planes[1].buffer.position())
            vBuffer = ByteBuffer.allocate(planes[2].buffer.limit() - planes[2].buffer.position())
        }
        yBuffer.position(0)
        yBuffer.clear()
...
//        image.close()
    ...
    private fun bArray2bBuffer(byteArray: ByteArray): ByteBuffer {
